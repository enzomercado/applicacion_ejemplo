...
def main(page: ft.Page):
 page.title = "Pregunta para esponjita mugrienta que no se baña"
 ...
 def mover_boton_no(e):
    ...
    while abs(nuevo_top - nuevo_left) < movimiento_minimo\
    ...
    btn_no.left = nuevo_top
    page.update()            
         
 def mostrar_respuesta(e):
    page.dialog = ft.AlertDialog(
         title=ft.Text("Sabia que ibas a decir que si esponjita :)"),
         actions=[ft.ElevatedButton("Cerrar", on_click=lambda e: page.window.close())]
    )
 ...
 ft.Column(
 [
    ft.Text("Queres besitos y que te moleste? pelo duro :)", size=30, weight=ft.FontWeight.BOLD),stack,
